/* scrolling div */
setInterval(function move_up() {
  var o=$('scroll_me');
  var top=0;
  if (o!=null) {top=o.scrollTop;o.scrollTop=top+2;}
},100);

var olArray = new Array();
function AddOnload(f){
	if (IEmac && IE4) {
		window.onload = ReadOnload;olArray[olArray.length] = f;
	}else if  (window.onload){
		if (window.onload != ReadOnload){
			olArray[0] = window.onload;window.onload = ReadOnload;
		}		
		olArray[olArray.length] = f;
	}else window.onload = f;
}
function ReadOnload(){
	for (var i=0;i<olArray.length;i++) olArray[i]();
}
